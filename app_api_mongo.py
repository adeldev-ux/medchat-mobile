"""MedChat AI – REST Microservice with MongoDB Atlas Chat Storage.

An AI microservice for the MedChat medical chatbot.
Receives messages from the client, processes them through
RAG (Vaccination/SPD) or LLM, stores chat history in
...
def _build_messages(user_msg: str, history: List[Dict]) -> List[Dict]:
    """
    Build full message list:
      1. Main system prompt (medical-only, hardened)
      2. Language-anchor system message (fixes Qwen language bleed)
      3. Last N cleaned history turns
      4. Current user message + [REPLY IN X ONLY] tag
    """
...
def _call_llm(user_msg: str, history: List[Dict]) -> Tuple[str, str]:
    """
...
app.config["SWAGGER"] = {
    ...
    "uiversion":   3,
    "description": "Medical RAG Chatbot – Vaccination & SPD",
    "version":     "5.0",
}
if Swagger:
...
def health():
    ...
    return jsonify(
        {
            "status":               "ok",
            "service":              "medchat-ai-microservice",
...
def chat():
      ...
      200:
        ...
        schema:
          ...
          properties:
            ...
            source:
              type: string
              enum: [vaccination_database, spd_database, ai_model]
            language:
              type: string
              enum: [ar, en]
            conversation_id:
    ...
    if topic == "vaccination":
        ...
        if result:
            reply  = vax_retriever.format(result, lang)
            source = "vaccination_database"

    if topic == "spd" and not reply:
        chunks = spd_retriever.search(user_msg, lang)
        if chunks:
            reply  = SPDRetriever.format(chunks, lang)
            source = "spd_database"

    if not reply:
        reply, source = _call_llm(user_msg, history)
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("  MedChat AI Microservice v5.0")
